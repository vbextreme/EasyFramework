...
__private void resource_set(guiResource_s* res){
	guiResource_s* old = rbhash_find(resources, res->name, 0);
	if( old ) rbhash_remove(resources, res->name, 0);
	rbhash_add(resources, res->name, 0, res);
}

__private guiResource_s* resource_new(const char* name, guiResource_e type){
	guiResource_s* res = mem_new(guiResource_s);
	if( !res ) err_fail("eom");
	res->name = str_dup(name,0);
	res->type = type;
	return res;
}
