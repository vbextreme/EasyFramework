...
void test_gui(__unused const char* argA, __unused const char* argB){
	...
	gui_s* loadResource = gui_simple_button_new(main, TESTEF_BUTTONRR_NAME, U8(TESTEF_BUTTONRR_NAME), button_themes);
	gui_simple_layout_table_add(main, loadResource, TESTEF_BUTTONRR_W, TESTEF_BUTTONRR_H, 1);
	loadResource->userdata = lbl;

	gui_s* loadTheme = gui_simple_button_new(main, TESTEF_BUTTONRT_NAME, U8(TESTEF_BUTTONRT_NAME), NULL);
